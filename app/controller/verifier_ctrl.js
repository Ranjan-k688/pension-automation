...
app.controller('verifier',function($scope,$http,sharedService) {
  $scope.pensionerData={};
  $scope.masterdata=[];
  $scope.postoffice=[];
  $scope.pensionerdetails={};
  $scope.verifierData={};
  $scope.verifierDataRet={};
  $scope.showdetails=[];
  $scope.frm= {};
  $scope.frm1= {};
  $scope.departments={};
  ...
  $scope.verifierDataFlag=false;
  $scope.verifierDataRetFlag=false;
...
$scope.fetchAllPensioners = function () {
  ...
  var req = {
    ...
    url: path+'/api/v1/pensioner-details',
  };
  
 $http(req).then(function(pensionerdetails_res){

     if (pensionerdetails_res) {
        $scope.pensionerdetails=pensionerdetails_res.data;
...
// $scope.fetchAllPensioners(); 

$scope.fetchAllPensionersData = function () {
  ...
  var req = {
    ...
    url: path+'/api/v1/pensioners/getAllPensionerData',
  };
  
 $http(req).then(function(PensionerData_res){

     if (PensionerData_res) {
        $scope.allPensionerData=PensionerData_res.data;
  ...
  $http.get(url,{
    ...
    headers: {
      ...
      if(response.status===200){
       ...
       if(status=='P'){
        $(".loaderbg").hide();
        $scope.verifierData=response.data.content;
        $scope.totalItems = response.data.totalElements; 
        if($scope.verifierData.length){
          $scope.verifierDataFlag=true;
          $scope.vermon=response.data.content[0].pension_month;
        $scope.veryr=response.data.content[0].pension_year;
        }
        
       }
       else if(status=='T'){
        $(".loaderbg").hide();
         $scope.verifierDataRet=response.data.content;
         $scope.totalItems = response.data.totalElements;
         if($scope.verifierDataRet.length){
          $scope.verifierDataRetFlag=true;
          $scope.vermonret=response.data.content[0].pension_month;
          $scope.veryrret=response.data.content[0].pension_year;
         }
...
$scope.checkCharacterLengthInPixels = function(val1) { 
  ...
  if (val1=='All') {
    if($scope.tab1flg){
      $scope.fetchforforward ('P'); 
    }
    else if($scope.tab2flg){
      $scope.fetchforforward ('T'); 
    }
 }else{
...
$scope.remarks=null;
$scope.sendSelectedPensioners = function(data,val, isReturn) {
  ...
  $http(req).then(
    function(res) {
      if (res.status == 200) {
        ...
        $('.popupbg').hide();
        if(val==='V'){
          alert("Forwarded for Disburserment!!");
        }
        else if(val==='R'){
          alert("Return for recheck!!");
        }
           $scope.remarks=null;
        ...
        if(!$scope.watchFlag){
          ...
          if($scope.tab1flg){
            $scope.fetchMonthlyPensionerData($scope.frm.searchCriteria,$scope.frm.searchCriteriaVal,$scope.search,'P');
            }
            else if($scope.tab2flg){
              $scope.fetchMonthlyPensionerData($scope.frm1.searchCriteria,$scope.frm1.searchCriteriaVal,$scope.search1,'T');
            }
        }
...
var requestData = {};
$scope.sendSelectedPensionersChk = function(data,val, isReturn) {
  var selectedData = data;
  var selectedFlag = isReturn ? 'selectAll' : 'selectRetunAll';
  var selectedKey = isReturn ? 'selected' : 'selectedReturn';
