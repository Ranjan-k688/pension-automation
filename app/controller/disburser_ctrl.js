...
app.controller('disburser',function($scope,$http,sharedService) {
  $scope.masterdata=[];
  $scope.postoffice=[];
  $scope.disbursedData={};
  $scope.disburserData={};
  $scope.disbursedDataFlag=false;
  $scope.disburserDataFlag=false;
  $scope.showdetails=[];
  $scope.frm= {};
  $scope.frm1= {};
  $scope.departments={};
...
$scope.fetchforforward = function(status) {
  ...
  $http.get(url,{
    ...
    headers: {
      ...
      if(status=='V'){
        $(".loaderbg").hide();
        $scope.disburserData=response.data.content;
        $scope.totalItems = response.data.totalElements;
        if($scope.disburserData.length){
          $scope.disburserDataFlag=true;
          $scope.dismon=response.data.content[0].pension_month;
          $scope.disyr=response.data.content[0].pension_year; 
        }
        
      }
      else if(status=='D'){
        $(".loaderbg").hide();
        $scope.disbursedData=response.data.content;
        $scope.totalItems = response.data.totalElements; 
        if($scope.disbursedData.length){
          $scope.disbursedDataFlag=true;
          $scope.disbmon=response.data.content[0].pension_month;
          $scope.disbyr=response.data.content[0].pension_year; 
        }
      }
    }
    else{
...
$scope.opentab=function(openflg){
  ...
  if(openflg=='tab1'){
    ...
    $scope.tab2flg=false;
    $scope.curPage = 0;
    // $scope.fetchforforward ('V');  
  }
  else if(openflg=='tab2'){
    $scope.tab1flg=false;
    $scope.tab2flg=true;
    $scope.curPage = 0;
    // $scope.fetchforforward ('D');
  }
      $scope.search = {};
      $scope.search1 = {};
      $scope.frm= {};
  ...
  $http(req).then(
    function(res) {
      if (res.status == 200) {
        ...
        $('.popupbg').hide();
        if(val==='D'){
          alert("Send for Disbursement!!");
        }
        else if(val==='T'){
          alert("Return to verifier!!");
        }
        $scope.remarks=null;
        if(!$scope.watchFlag){
          if($scope.tab1flg){
          ...
          if($scope.tab1flg){
            $scope.fetchMonthlyPensionerData($scope.frm.searchCriteria,$scope.frm.searchCriteriaVal,$scope.search,'V');
            }
            else if($scope.tab2flg){
              $scope.fetchMonthlyPensionerData($scope.frm1.searchCriteria,$scope.frm1.searchCriteriaVal,$scope.search1,'D');
            }
        }
          $scope.selectAll = false;
          $scope.selectReturnAll = false;
