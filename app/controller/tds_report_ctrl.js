...
app.controller('tdsReport', function ($scope, $http, $rootScope, numberToWordsService) {
...
$scope.downloadAdviceExcel = function (val1) {
  if(val1.year!==undefined && val1.month!==undefined){
    ...
    var req = {
      ...
      url: `${path}/api/v1/tds-hra-report/download/tds?pensionMonth=${val1.month}&pensionYear=${val1.year}`,
    ...
    $http(req).then(function (response) {
        if (response.status===200) {
          ...
          $scope.monPensioerReportTotal = response.data.totalTds;
          if($scope.monPensioerReportExcel.length>0){
                setTimeout(function () {
                  $(".excelformate").table2excel({
                      filename: "TDS-Report_"+$scope.getMonthName(val1.month)+"-"+val1.year+".xls"
                  });
                  $(".generateload").hide();
                  location.reload();
  ...
  if (val1.year !== undefined && val1.month !== undefined) {
    ...
    $http(req).then(function (response) {
      console.log(response);
        if (response.status===200) {
          $scope.monPensioerReportExcel = response.data.report;
          console.log($scope.monPensioerReportExcel);
          $scope.monPensioerReportTotal = response.data.totalTds;
  ...
  //                 // Wait for a short delay to allow the table to render properly before generating the PDF
                  setTimeout(function() {
                    ...
                    if (printWindow) {
                        ...
                        printWindow.document.write('<html><head><title>TDS Report_'+ $scope.getMonthName(val1.month)+'_'+val1.year+' </title></head><body>');
